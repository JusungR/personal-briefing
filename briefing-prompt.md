# 일일 브리핑 루틴

## 날짜 계산 (최우선)

시스템이 제공하는 `currentDate`는 UTC 기준 날짜이다.
한국 시간(KST, UTC+9)으로 오늘 날짜를 먼저 계산한다.

- UTC 기준 15:00 **이후**이면 한국은 이미 다음 날  
  (15:00 UTC = 00:00 KST 다음 날)
- `currentDate`에 시각 정보가 없는 경우 → 아침 브리핑 기준  
  (KST 06:00~10:00 = UTC 전날 21:00~01:00)이므로  
  **KST 날짜 = `currentDate` + 1일** 을 기본값으로 사용
- 예) `currentDate` = 2026-05-27 → KST 브리핑 날짜 = **2026-05-28**

이하 모든 날짜·일정·뉴스 조회는 **KST 오늘 날짜** 기준으로 동작한다.

---

오늘의 일일 브리핑을 작성한다. 한국어, 간결한 문장.
세 개의 섹션을 순서대로 출력하고, 마지막에 전체를 하나의
정리된 메시지로 한 번 더 모아서 마무리한다.

## 1. 날씨 (서울)

WebSearch로 오늘 서울 날씨 조회:
- 최저/최고 기온, 강수 확률, 미세먼지·초미세먼지 등급
- 한 줄 권고 (우산/겉옷/마스크 여부)

## 2. 오늘 일정

Google Calendar의 `list_events`로 오늘 00:00 ~ 24:00 (Asia/Seoul) 범위의 이벤트 조회.
- `startTime`: KST 오늘 날짜 `T00:00:00+09:00`
- `endTime`:   KST 오늘 날짜 `T23:59:59+09:00`
- `timeZone`: `Asia/Seoul`
- 시간순 나열: "HH:MM  제목  (위치 또는 회의 링크)"
- 종일 일정: "[종일] 제목"
- 일정이 비어 있으면 "오늘 등록된 일정 없음"

## 3. 간밤 미국·유럽 주요 뉴스

어제 현지 시간 기준 발생한 주요 뉴스를 WebSearch로 조회.
- 미국 최대 5건, 유럽 최대 5건
- 카테고리: 정치, 경제, 산업·기술, 시장. 가십·연예·스포츠 제외.
- 1차 출처(로이터, AP, FT, BBC, Bloomberg, WSJ 등) 우선.
- 항목 형식:
  **헤드라인 (한국어)**
  1~2문장 요약. 출처: 매체명

검색 시 같은 사건이 여러 매체에 나오면 1차 출처 한 곳만 인용.
미확인·추측성 보도는 "보도됨", "~로 전해졌다" 같은 표현으로 단정 회피.
