...
WebSearch로 오늘 서울 날씨 조회. 불릿(`•`) 4줄 고정 형식:
...
- 기온은 정수 표기. "약" 붙이지 않는다.
- 미세먼지 등급: 좋음/보통/나쁨/매우나쁨. 조회 불가 시 "미확인".
- 표(table) 형식 사용 금지. 습도·UV·URL 등 부가 정보 생략.
...
- `timeZone`: `Asia/Seoul`
- 시간순 나열: `• HH:MM ~ HH:MM 제목 (위치 또는 회의 링크)`
  - 시작·종료 시간을 `~`로 구분. 종료 시간이 없으면 시작 시간만 표기.
  - `-` 하이픈 구분자 사용 금지.
- 종일 일정: `• [종일] 제목`
...
- **지역별로 먼저 그룹화**한 뒤, 지역 내에서 카테고리 구분 없이 중요도순 나열:
  - `### 🇺🇸 미국` → 해당 지역 뉴스
  - `### 🇪🇺 유럽` → 해당 지역 뉴스
- 카테고리 소제목(`【】`, `[경제]`, `🔬 산업·기술` 등)은 사용하지 않는다.
- 분야(정치, 경제, 산업·기술, 시장)를 균형 있게 포함하되 헤더로 나누지 않는다. 가십·연예·스포츠 제외.
- 항목 형식:
  **헤드라인 (한국어)**
  1~2문장 요약. 출처: 매체명
...
- 마크다운 테이블(`| |`) 형식 사용 금지. 위의 텍스트 리스트 형식만 사용한다.
...
- 본문: 아래 **정규 템플릿**의 구조를 그대로 따라 구성한 뒤, **맨 아래에 뉴스 원문 링크 목록을 추가**한다.
- 발송 도구: `mcp__Gmail__create_draft` (Gmail 드래프트 생성)

**정규 출력 템플릿 (이 구조를 반드시 따른다):**
...
• HH:MM ~ HH:MM 제목 (위치 또는 회의 링크)
• [종일] 제목
(일정이 없으면: "• 오늘 등록된 일정 없음")
...
## 📊 이번 주 경제지표 ({KST_TODAY} ~ {KST_TODAY+6일})
...
> **형식 고정 규칙:** 제목은 반드시 `[Daily Briefing] {KST_TODAY}` 형식이며,
> 섹션 구분은 `---`만 사용한다. Unicode 박스 라인(`━━━`), 번호 매기기(`1.`, `2.`),
> 전각 괄호(`【】`), 카테고리 소제목, 마크다운 테이블을 사용하지 않는다.
