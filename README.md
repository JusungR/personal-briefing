...
- **스키마**: `country,indicator,release_date,release_time_kst,period,level,yoy,mom,forecast,previous,source`
- **발표값은 세 칸으로 나눈다**: `level`(지수 원계열) · `yoy`(전년비) · `mom`(전월비).
...
- **미래 일정은 저장하지 않는다** — `level`·`yoy`·`mom`이 모두 비면 거부된다. 일정은
  매일 바뀌므로 보존 가치가 없다.
